...
async def _(hazmat):
    await hazmat.edit("`Rizz Ingin Mengaktifkan Perintah Hazmat ⚡`")
    level = hazmat.pattern_match.group(2)
    if hazmat.fwd_from:
        return
    if not hazmat.reply_to_msg_id:
        await hazmat.edit("`Rizz, Mohon Balas Ke Sticker/Gambar ⚡`")
        return
    reply_message = await hazmat.get_reply_message()
    if not reply_message.media:
        await hazmat.edit("`Kata Bisa Menghancurkan Apapun Rizz ⚡`")
        return
    ...
    async with hazmat.client.conversation(chat) as conv:
        try:
            ...
            await hazmat.reply("`Rizz, Mohon Buka Blokir` @hazmat_suit_bot `Lalu Coba Lagi`")
            return
        if response.text.startswith("I can't"):
            await hazmat.edit("`Mohon Maaf Rizz, GIF Tidak Bisa...`")
