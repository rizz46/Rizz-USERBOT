                ...
                if i.sender == str(guser.id):
                    ...
                    if admin or creator:
                        try:
                            ...
                            await tele.reply(
                                f"**Rizz, Pengguna Gban Telah Bergabung** \n"
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Rizz Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`proses Global Banned Pengguna ಠಿ_ಠ`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Rizz 🤪`")
    ...
    if user:
        if user.id == 1353102497:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke Lord Aris, Dia Adalah Pembuat Saya ヅ`"
            )
        try:
...
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(f"`Memulai Membatalkan Perintah Global Banned, Pengguna Ini Akan Dapat Bergabung Ke Grup Anda Rizz ヅ`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        ...
        await dark.edit("`Harap Balas Ke Pesan Pengguna Rizz ヅ`")
    try:
