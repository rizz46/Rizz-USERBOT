...
async def kickme(leave):
    ...
    await leave.edit("`Rizz Telah Meninggalkan Grup ⚡`")
...
async def unmute_chat(unm_e):
    ...
    await unm_e.edit("```Rizz telah membuka obrolan, Obrolan Tidak Lagi Dibisukan```")
    await sleep(2)
    await unm_e.delete()
...
async def mute_chat(mute_e):
    ...
    await mute_e.edit("`⚡ Rizz Telah Membisukan Obrolan!`")
    await sleep(2)
    await mute_e.delete()
    if BOTLOG:
